<template>
  <div class="portfolio-container" :style="`min-height: 100vh !important;`">
    ...
    <div class="top-container" :style="`height: 100vh !important;`">
      <transition name="fast-fade" mode="out-in">
        <template v-show="genre === 'year'">
          <portfolio-title
            key="year"
            selectGenre="year"
        ...
        <template v-show="genre === 'category'">
          <div key="category">
            <portfolio-title
            ...
            />
            <!-- <client-only>
                            <div v-if="genre !== 'year'" class="flex justify-center">
    ...
    <!-- <template v-if="filterContents && filterContents.length > 0"> -->
    <!-- <div
            ...
            :style="`height: 100vh;`">
                ...
                <div class="text-container">
                    ...
                    <span v-show="content.fields.github" class="icon-container github">
                        <tooltip
                        class="icon-wrapper"
                        bottom
                        text="GitHub"
...
export default Vue.extend({
  ...
  methods: {
    ...
    getSelectedSkill(): Entry<Skill> | null {
      ...
      // console.log((this as unknown as {skills: Entry<Skill>[]}).skills)
      if (skills) {
        if (this.$route.query.id) {
          const filter = skills.filter(
            (item) => item.fields.id === this.$route.query.id
          );
      ...
      if (process.client) {
        ...
        if (bp === "") {
          ...
          if (Number(width) <= Number(breakpoint["xs"])) {
            ...
            Number(width) > Number(breakpoint["xs"]) &&
            Number(width) <= Number(breakpoint["sm"])
          ) {
            bp = "sm";
          } else if (
            Number(width) > Number(breakpoint["sm"]) &&
            Number(width) <= Number(breakpoint["md"])
          ) {
            bp = "md";
          } else if (
            Number(width) > Number(breakpoint["md"]) &&
            Number(width) <= Number(breakpoint["lg"])
          ) {
            bp = "lg";
          } else if (
            Number(width) > Number(breakpoint["lg"]) &&
            Number(width) <= Number(breakpoint["xl"])
          ) {
            bp = "xl";
          } else if (
            Number(width) > Number(breakpoint["xl"]) &&
            Number(width) <= Number(breakpoint["2xl"])
          ) {
            bp = "2xl";
